...
In this example, we will analyze the United States GDP time series. We will use the `acfinter()` function to see the ACF, PACF, Box_Pierce, Pv_Box, Ljung_Box, and Pv_Ljung. We can also see the normality analysis and stationarity analysis. Finally, a plot with ACF, PACF, and Pv LJUNG BOX.
...
As you can see, the `result` variable lists three elements: ACF-PACF Test, Stationary Test, and normality test. Concerning the first element, we can see in ACF that every lag is statistically significant, which means that the GDP time series could not be stationary
